...
Note: Execution history in $HOME/.charge/sessions/ will NOT be deleted.
To delete sessions, use: rm -rf $HOME/.charge/sessions/{session_id}
...
1. Check if `$HOME/.charge/workflows/` is now empty
2. If empty, delete the workflows directory
3. Check if `$HOME/.charge/` has no other content (no workflows or sessions)
4. If empty, delete `$HOME/.charge/` (or leave it for future use)
...
- To delete sessions/executions, use UNIX commands:
  - `rm -rf $HOME/.charge/sessions/{session_id}` - delete entire session
  - `rm -rf $HOME/.charge/sessions/{session_id}/{execution}` - delete specific execution
